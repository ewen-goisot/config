...
if has('nvim')
	...
	tnoremap <expr> <c-s><c-r> '<c-\><c-n>"'.nr2char(getchar()).'pi'
	tnoremap <c-s><c-s> <c-s>
	tnoremap <c-s><Esc> <Esc>
	"tnoremap <c-s>p <c-\><c-n>pi
	tnoremap <c-s><c-p> <c-\><c-n>pi
	tnoremap <c-s><c-m> <c-e> \<c-m>
	"be carefull: symbol depends on shell prompt, using $ is safer
	tnoremap <c-s><c-y> <c-\><c-n>0f2<Right>y$i
	"tnoremap <Up> <c-p>
	"tnoremap <Down> <c-n>
else
...
noremap "_ "_
cnoremap <c-r>_ <c-r>_
inoremap <c-r>_ <c-r>_
noremap @? @*
cnoremap <c-r>? <c-r>*
inoremap <c-r>? <c-r>*
...
" <21-05-31> deoplete features aren't the priority
...
" dialog colors for .not files
function! NotDial()
	noremap <buffer> g0 :'{+1,'}-1s/^\(\d>\)\?//\|set tw=78<c-m>gwap:'{+1,'}-1s/^/0>/\|set tw=80<c-m>
	noremap <buffer> g1 :'{+1,'}-1s/^\(\d>\)\?//\|set tw=78<c-m>gwap:'{+1,'}-1s/^/1>/\|set tw=80<c-m>
	noremap <buffer> g2 :'{+1,'}-1s/^\(\d>\)\?//\|set tw=78<c-m>gwap:'{+1,'}-1s/^/2>/\|set tw=80<c-m>
	noremap <buffer> g3 :'{+1,'}-1s/^\(\d>\)\?//\|set tw=78<c-m>gwap:'{+1,'}-1s/^/3>/\|set tw=80<c-m>
	noremap <buffer> g4 :'{+1,'}-1s/^\(\d>\)\?//\|set tw=78<c-m>gwap:'{+1,'}-1s/^/4>/\|set tw=80<c-m>
	noremap <buffer> g5 :'{+1,'}-1s/^\(\d>\)\?//\|set tw=78<c-m>gwap:'{+1,'}-1s/^/5>/\|set tw=80<c-m>
	noremap <buffer> g6 :'{+1,'}-1s/^\(\d>\)\?//\|set tw=78<c-m>gwap:'{+1,'}-1s/^/6>/\|set tw=80<c-m>
	noremap <buffer> g7 :'{+1,'}-1s/^\(\d>\)\?//\|set tw=80<c-m>gwap
endfunction
...
" 2 non canonique
nnoremap <silent> <Right> :call Move(1)<cr>
nnoremap n l
vnoremap n l
onoremap n l
...
"be carefull with next lines: some chars may be invisible
noremap ↦ (
noremap ̊  )
noremap ̄  <
onoremap ̄  <
