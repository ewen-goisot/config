...
let s:keymap += [['FocusTarget','i','Set Focus back to the editor']]
let s:keymap += [['ClearHistory',"'",'Clear undo history (with confirmation)']]
let s:keymap += [['TimestampToggle','+','Toggle relative timestamp']]
let s:keymap += [['DiffToggle','-','Toggle the diff panel']]
let s:keymap += [['NextState','m','Move to the next undo state']]
let s:keymap += [['PreviousState','s','Move to the previous undo state']]
let s:keymap += [['NextSavedState','t','Move to the next saved state']]
let s:keymap += [['PreviousSavedState','n','Move to the previous saved state']]
let s:keymap += [['Redo','.','Redo']]
let s:keymap += [['Undo',',','Undo']]
let s:keymap += [['Enter','<2-LeftMouse>','Move to the current state']]
let s:keymap += [['Enter','u','Move to the current state']]
...
function! s:undotree.BindKey() abort
	...
	nnoremap <silent> <buffer> <lt> zh
	nnoremap <silent> <buffer> > zl
    nnoremap <silent> <buffer> q. <c-w>p
